...
class GenModelv0(tf.keras.Model):
    ...
    def from_config(cls, config):
        ...
        cls.gru = cls.gru = tf.keras.layers.GRU(
            config["rnn_units"],
            return_sequences=True,
            return_state=True)
...
class OneStep(tf.keras.Model):
    ...
    def generate_one_step(self, inputs, states=None):
        # Conver strings to token IDs.
        inputs = str(inputs)
        inputs_chars = tf.string.unicode_split(
            inputs, 'UTF-8')
        ...
        # Apply the prediction mask: preven "[UNK]" from being
        # generated.
...
def get_model(file_path):
    model = GenModelv0(
        # Be sure the vocabulary size mathes the 'StringLookup' layers.
        vocab_size=len(ids_from_chars.get_vocabulary()),
        embedding_dim=EMBEDDING_DIM,
        rnn_units=RNN_UNITS)
    loss = tf.losses.SparseCategocalCrossentropy(from_logits=True)
    metrics = [
